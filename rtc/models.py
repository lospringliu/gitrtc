...
class Snapshot(MPTTModel):
	...
	def update_baselines(self):
		items = json.loads(shell.getoutput("%s list baselines -s %s -r rtc -m 1000 -j" % (scmcommand, self.uuid), clean=False))
		for item in items:
			if type(item) == type({}) and 'baselines' in item.keys():
...
class Stream(MPTTModel):
	...
	def post_migrate_actions(self,rtcdir='',manual=False,post_incremental=False):
		if not rtcdir:
	...
	def update_baselines(self,post_incremental=False):
		...
		items = json.loads(shell.getoutput("%s list baselines -r rtc -w %s -C %s -j -m 1000" % (scmcommand, self.uuid, self.component.uuid), clean=False))
		for item in items:
			baselinep = None
			baselines = []
			if type(item) == type({}) and 'baselines' in item.keys():
...
class Workspace(models.Model):
	...
	def ws_list(self,verbose=False,component=None,stream=None):
		...
		return list(filter(lambda x: x['name'] == self.name, json_r))
